...
## 当前的首要任务
现在检测脚本的书写还是有问题，老师给出了以下修改：
1. 添加 backup_dir 的方式不对，现在只是把 backup_dir 放在了 **kwargs 中，并没有用到 backup_dir。添加 backup_dir 的作用，是为了指定缓存当前任务需要的 **.json 文件的
2. 代码格式不规范，要参考`https://zhuanlan.zhihu.com/p/575965662`和`D:\android_template\AppSim\.ruff.toml`