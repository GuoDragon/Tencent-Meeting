...
import com.example.tencent_meeting_sim.presentation.contact.FriendsDetailsContract
...
class FriendsDetailsPresenter(
    ...
    /**
     * Load friend info by userId (for Navigation support)
     * @param userId User ID
     */
    fun loadFriendInfoById(userId: String) {
        presenterScope.launch {
            try {
                view?.showLoading()
                val user = dataRepository.getUserById(userId)
                if (user != null) {
                    view?.showFriendInfo(user)
                } else {
